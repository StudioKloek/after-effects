...
module.exports = {
  ...
  execute : function(command) {
    ...
    try {
      execSync('afterfx.exe', {cwd: jsx.program_path, timeout:10000});
    } catch (err) {
    ...
    .then( jsx => new Promise( resolve => {
      exec(`afterfx.com -r ${jsx.script}`, {cwd: jsx.program_path, stdio: STDIO}, (/*err, stdout, stderr*/) => {
        // see * below
  ...
  executeSync : function(command) {
    ...
    //Execute JSX
    try {
      execSync(`afterfx.com -r ${jsx.script}`, {cwd: jsx.program_path, stdio: STDIO});
    } catch (err) {
      //TODO *
